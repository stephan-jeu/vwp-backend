...
A lot of times we'll have more researchers available for a visit than required. In that case the following criteria should be used to order the researchers:
1 - Travel time: using google maps api we should calculate the travel time from the researcher's current location to the visit location and set a 'travel' value incrementing for eache subsequent 15 minutes. So travel time 0-15 minutes is 1, 15-30 2, 30-45 3, 45-60 4, 60-75 6 and higher is excluded. We should normalize the value by dividing by 6 (all values should be between 0 and 1). The weight of this criteria is 4
2 - already assigned: number of already assigned visits / total available capacity for candidate researcher. The weight of this criteria is 3
3 - number of assigned visits where number of researchers > 2 / total number of planned visits where number of researchers > 2 for candidate researcher. The weight of this criteria is 3
4 - number of visits that require fiets / total number of visits that require fiets for candidate researcher. The weight of this criteria is 1
5 - number of already assigned visits for this researcher to this project / by total number of visits for this project. The weight of this criteria is 1
