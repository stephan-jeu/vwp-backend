...
async def test_ordering():
    ...
    for v in visits:
        print(
            f"  Visit {v.visit_nr}: Date={v.from_date}, Protocols={[p.id for p in v.protocols]}"
        )
        # Check if multiple indices
        _ = [p.visit_index for p in getattr(v, "_debug_reqs", [])]
    ...
    if len(visits) == 1:
        print(
            "FAILURE: Both protocol visits grouped into ONE visit (Gap=0 allowed this)."
        )
    elif len(visits) == 2:
        v1 = visits[0]
        v2 = visits[1]
