...
async def debug_visit(visit_id: int):
    ...
    async with AsyncSessionLocal() as db:
        ...
        print(
            f"Flags: WBC={visit.wbc}, Fiets={visit.fiets}, HUB={visit.hub}, DVP={visit.dvp}, Sleutel={visit.sleutel}"
        )
        ...
        if status != 10 and str(status) != "open":
            print(
                f"WARNING: Visit is NOT OPEN ({status}). It will be skipped by planner."
            )
        ...
        users = (
            (await db.execute(select(User).order_by(User.full_name))).scalars().all()
        )
        ...
        for u in users:
            ...
            if qualifies:
                ...
                if has_cap:
                    ...
                    print(
                        f"[NO_CAP] {u.full_name} ({u.id}) - {cap_str} (Needed: {pod})"
                    )
                ...
                if any("vliegroute" in (f.name or "").lower() for f in visit.functions):
                    if not u.vrfg:
                        failed_reasons.append("Missing 'vrfg'")
