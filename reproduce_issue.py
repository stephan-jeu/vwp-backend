...
async def main():
    async with AsyncSessionLocal() as db:
        ...
        for u in users:
             print(f"  User: {u.id} - {u.full_name} (Roofvogel: {getattr(u, 'roofvogel', '?')})")
        ...
        for r in rows:
            print(f"  Row: user_id={r.user_id}, morning={r.morning_days}, day={r.daytime_days}, night={r.nighttime_days}, flex={r.flex_days}")
        ...
        if rows_raw:
             ...
             print(f"  getattr(row, 'user_id'): {getattr(rows_raw[0], 'user_id', 'FAIL')}")
        ...
        if not result:
            ...
            for family, parts in result.items():
                for part, cap in parts.items():
                    print(f"  {family} - {part}: Required={cap.required}, Assigned={cap.assigned}, Shortfall={cap.shortfall}, Spare={cap.spare}")
