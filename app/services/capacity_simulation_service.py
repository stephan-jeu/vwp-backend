...
async def _load_initial_protocol_states(
    ...
    stmt = (
        select(ProtocolVisitWindow.protocol_id, func.max(Visit.to_date))
        .join(
        ...
        .where(and_(Visit.researchers.any(), Visit.to_date < start_date))  # Locked
...
async def generate_and_store_simulation(
    db: AsyncSession, start_monday: date | None = None
) -> SimulationResult:
    """
    ...
    def add_deadline_result(v: Visit, is_planned: bool):
        ...
        if is_planned:
            part_dict[deadline] = SimulationResultCell(
                current.planned + len(v.researchers), current.unplannable
            )
        else:
            req = v.required_researchers or 1
            part_dict[deadline] = SimulationResultCell(
                current.planned, current.unplannable + req
            )
    ...
    while current_monday <= horizon_end:
        ...
        # 1b. Skipped visits (unplannable for this week)
        ...
        tot_row[week_iso_id] = {
            "spare": total_spare,
            "planned": total_planned_this_week,
        }
        ...
        active_groups = (
            set(deadline_results.keys())
            | set(week_planned_map.keys())
            | mandatory_groups
        )
    ...
    full_json = {
        ...
        "week_view": {"weeks": simulated_weeks, "rows": week_view_rows},
    }
    ...
    if existing:
        ...
        sim_res = SimulationResult(
            horizon_start=horizon_start, horizon_end=horizon_end, grid_data=full_json
        )
        db.add(sim_res)
...
async def simulate_capacity_planning(
    ...
    return CapacitySimulationResponse(
        horizon_start=res.horizon_start, horizon_end=res.horizon_end, grid=final_grid
    )
...
async def simulate_week_capacity(
    ...
    selection_result = await select_visits_cp_sat(
        db, week_monday, timeout_seconds=None, include_travel_time=False
    )
    ...
    def add_stats(visits: list[Visit], is_assigned: bool):
        for v in visits:
            ...
            if cell is None:
                cell = FamilyDaypartCapacity(
                    required=0, assigned=0, shortfall=0, spare=0
                )
