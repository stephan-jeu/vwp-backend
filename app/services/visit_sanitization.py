...
async def sanitize_future_planning(
    db: AsyncSession, 
    week_monday: date, 
    newly_planned_visit_ids: list[int]
) -> list[int]:
    """
    ...
    lookahead_start = current_week + 1
    lookahead_end = current_week + 8
    ...
    stmt_future = (
        ...
        .join(visit_protocol_visit_windows, visit_protocol_visit_windows.c.visit_id == Visit.id)
        .join(ProtocolVisitWindow, ProtocolVisitWindow.id == visit_protocol_visit_windows.c.protocol_visit_window_id)
    ...
    for (visit_future, prot_id, min_val, min_unit) in results:
        ...
        if days_diff < required_gap_days:
            ...
            _logger.warning(
                ...
                visit_future.id, visit_future.planned_week, prot_id, min_val, min_unit, current_week
            )
