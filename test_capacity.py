...
def test():
    ...
    for i in range(7):
        ...
        active = next(
            (p for p in patterns if p.start_date <= day_date <= p.end_date), None
        )
        if not active:
            continue
    ...
    print(
        f"morning_days: {morning_days}, max_mornings: {max_mornings}, returned: {min(morning_days, max_mornings)}"
    )
