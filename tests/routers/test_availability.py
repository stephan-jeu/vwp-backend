...
async def test_get_my_availability_empty(
    async_client: AsyncClient, override_deps, mocker
):
    # Mock the service function directly to return empty list
    mocker.patch("app.routers.availability.get_user_availability", return_value=[])
...
async def test_get_my_availability_with_data(
    async_client: AsyncClient, override_deps, mocker, test_user
):
    # Mock data
    ...
    # Patch service
    mocker.patch(
        "app.routers.availability.get_user_availability", return_value=[week1, week2]
    )
...
async def test_get_my_availability_pass_params(
    ...
    mock_service = mocker.patch(
        "app.routers.availability.get_user_availability", return_value=[]
    )

    await async_client.get(
        "/availability/me", params={"week_start": 20, "week_end": 25}
    )
