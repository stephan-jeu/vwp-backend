...
async def test_max_travel_time_configuration():
    ...
    with (
        patch("app.services.travel_time.get_travel_minutes_batch") as mock_get_travel,
        patch(
            "app.services.visit_selection_ortools.get_settings"
        ) as mock_settings_getter,
        patch(
        ...
        mock_get_travel.return_value = {
            ("UserLocation", "VisitLocation, ProjectLoc"): 70
        }
        ...
        assert visit in result_loose.selected, (
            "Visit should be scheduled with loose travel limit"
        )
