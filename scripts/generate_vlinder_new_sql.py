...
OUT_PATH = os.path.join(OUT_DIR, "seed_vlinder.sql")
...
def main() -> None:
    ...
    stmts: list[str] = []
    stmts.append("-- Seed generated for family Vlinder protocols")
    ...
    # Ensure species
    species = ["Iepenpage", "Grote vos", "Teunisbloempijlstaart"]
    for sp in species:
        stmts.append(
    ...
    # Shared constraints for these protocols
    visits = 3
    duration_hours = 2.0
    min_gap_days = 7
    max_wind_bft = 4
    ...
    # Start-time condition text (kept in start_time_condition for clarity)
    start_condition = "Tussen 10:00 en 15:00 starten (evt. om 09:00 starten als het dan al 22 graden is en zonnig)"

    # Temperature condition text captured in visit_conditions_text
    temp_condition = (
        "Min. 15 tot 19 graden (<50% bewolking) of vanaf 20 graden (>50% bewolking)"
    )

    def add_protocol_with_windows(
        species_name: str, windows: list[tuple[str, str]]
    ) -> None:
        vals = (
            ...
            "NULL, NULL, "  # start by condition text rather than relative/absolute
            "NULL, NULL, NULL, NULL, "
            "NULL, "
            f"{to_sql_null_or_int(max_wind_bft)}, {to_sql_null_or_str(max_precip)}, "
            f"{to_sql_null_or_str(start_condition)}, NULL, {to_sql_null_or_str(temp_condition)}, false, false, false, false, NULL"
        )
        stmts.append(
    ...
    # Iepenpage – 3 identical windows 1 Jun–15 Jul
    add_protocol_with_windows(
        species_name="Iepenpage",
        windows=[("2000-06-01", "2000-07-15")] * 3,
    )

    # Grote vos – 3 identical windows 15 May–15 Jun
    add_protocol_with_windows(
        species_name="Grote vos",
        windows=[("2000-05-15", "2000-06-15")] * 3,
    )

    # Teunisbloempijlstaart – 2 x Jul, then 1–15 Aug
    add_protocol_with_windows(
        species_name="Teunisbloempijlstaart",
        windows=[
